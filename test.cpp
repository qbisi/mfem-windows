...
int main(int argc, char *argv[])
{
   ...
   real_t modulus_E = 2.1e5;            //Young's modulus E = 210 GPa
   real_t modulus_v = 0.33;             //Poisson's ratio v = 0.33
   real_t lame_lamda = modulus_v*modulus_E/((1+modulus_v)*(1-2*modulus_v));
   real_t lame_mu = modulus_E/(2*(1+modulus_v));
   lame_lamda=2*lame_lamda*lame_mu/(lame_lamda+2*lame_mu);
   Vector lambda(pmesh->attributes.Max());
   //lambda(0) = lame_lamda;
   //lambda(1) = 2*lame_lamda;
   lambda = lame_lamda;
   ...
   //a->AddDomainIntegrator(new ReferneceIntegrator(lambda_func, mu_func));
  /* cout<<"GetAttribute(i) = \n";
   for (int i = 0; i < fespace -> GetNE(); i++)
   {
      const int elem_attr = fespace->GetMesh()->GetAttribute(i); 
      cout<<elem_attr<<" ";
   }
   cout<<endl;*/

   Array<int> set_marker;
   set_marker.SetSize(pmesh->attributes.Max());
   set_marker = 0;
   set_marker[0] = 1; 
   set_marker.Print();  cout<<endl;
   a->AddDomainIntegrator(new ElasticityIntegrator(lambda_func, mu_func), set_marker);
   Array<int> set_marker1;
   set_marker1.SetSize(pmesh->attributes.Max());
   set_marker1 = 0;
   set_marker1[1] = 1;   
   a->AddDomainIntegrator(new ElasticityIntegrator(lambda_func, mu_func), set_marker1);
   set_marker1.Print();  cout<<endl;
   ...
   if(myid == 0)cout<<"****DEBUG b external_work.Normlinf() = "<<b->Normlinf()<<endl;
   if(myid == 0)cout<<"****DEBUG B external_work.Normlinf() = "<<B.Normlinf()<<endl;

   // 14. Define and apply a parallel PCG solver for A X = B with the BoomerAMG
   //     preconditioner from hypre.
   MUMPSSolver *matrix_equation_solver = new MUMPSSolver(A);
   matrix_equation_solver->SetMatrixSymType(MUMPSSolver::MatType::UNSYMMETRIC ); 
   matrix_equation_solver->SetPrintLevel(2);
   matrix_equation_solver->Mult(B, X);  
   delete  matrix_equation_solver; 
   if(myid == 0)cout<<"****DEBUG X.Norml2() = "<<X.Norml2()<<endl;
   if(myid == 0)cout<<"****DEBUG X.Normlinf() = "<<X.Normlinf()<<endl;
